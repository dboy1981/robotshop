  ...
  async stepperAction() {
    ...
    if (data.qty > stock) {
      ...
      const datas = {
        id: goods.id,
        qty: data.qty,
        price: this.caculatePrice(Number(goods.days) > 0 ? goods.zujin : goods.unit_price, goods.yajin, Number(goods.days), Number(data.qty))
      };
  ...
  async addcartAction() {
    ...
    for (const val of arr) {
      ...
      if (think.isEmpty(goods.suk)) {
        ...
        dataobj.zujin = Number(goods.zujin);
        dataobj.yajin = Number(goods.yajin);
        dataobj.price = this.caculatePrice(Number(val.days) > 0 ? dataobj.zujin : dataobj.unit_price, dataobj.yajin, Number(val.days), Number(val.qty));
      } else {
        ...
        dataobj.zujin = Number(getpr.sku_zujin);
        dataobj.yajin = Number(getpr.sku_yajin);
        dataobj.price = this.caculatePrice(Number(val.days) > 0 ? dataobj.zujin : dataobj.unit_price, dataobj.yajin, Number(val.days), Number(val.qty));
